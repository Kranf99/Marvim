...
// matched against users.apitoken in MarvinUsers.sqlite to identify the calling user.
//
// {"mode":"create","data":[{"category":102,"schema":"F:/","name":"census-income.zip",
//   "server":"THUNDERBOLT","department":"Sales","columns":["age","class of worker", ...]}]}
//
...
foreach ($data as $entry)
{
    ...
    {
        $idAsset = (int)$row['id'];
        $stmt = $db->prepare('UPDATE Assets SET category=:cat, idDepartment=:dep, dateUpdated=:u WHERE id=:id');
    ...
    {
        $stmt = $db->prepare('INSERT INTO Assets(idDepartment,category,schema,idserver,name,status,popularity,rating,idowner,dateCreated,dateUpdated) '.
            'VALUES(:dep,:cat,:sc,:s,:n,0,0,0,:o,:c,:u)');
        $stmt->bindValue(':dep', $idDept);
        $stmt->bindValue(':cat', $category);
        $stmt->bindValue(':sc', $schema);
        $stmt->bindValue(':s', $idserver);
    ...
    foreach ($columns as $colName)
    {
        ...
        $stmt = $db->prepare('INSERT INTO Columns(idasset,name,status,popularity,rating,idowner,dateCreated,dateUpdated) '.
            'VALUES(:a,:n,0,0,0,:o,:c,:u)');
        $stmt->bindValue(':a', $idAsset);
