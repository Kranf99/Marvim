<?php
/**
 * extract_api.php — Anatella script extraction, persisted directly in MarvinDB.
 *
...
try { switch ($action) {
    ...
    case 'save_script':
        ...
        if ($idServer <= 0) {
            http_response_code(400);
            echo json_encode(array('error' => 'idServer is required (select a Marvin server before extraction)'));
            exit;
        }
    ...
    // ── List Marvin servers (extraction target selector) ──────────────────
...
/** Persist one extraction result into MarvinDB. Returns the script asset id. */
