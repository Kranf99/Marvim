<?php
/**
 * lineage_store.php — shared write layer for lineage data in MarvinDB.sqlite.
 *
...
function lin_marvinDbPath() {
    $env = getenv('MARVIN_DB');                 // test override
    return $env !== false && $env !== '' ? $env : __DIR__ . '/../../../db/MarvinDB.sqlite';
}

function lin_marvinPdo() {
    $pdo = new PDO('sqlite:' . lin_marvinDbPath());
...
function lin_now() {
    return date('Ymd H:i:s');   // marvin-native timestamp format
}

/**
 * Default department applied to assets created by the extractor.
 * Marvin list pages INNER JOIN userDepartmentRights on idDepartment, so a
...
/**
 * Owner (MarvinUsers id) applied to assets created/updated by the extractor.
...
/**
 * Ensure the MarvinDB lineage schema delta exists. Safe to call on every request.
...
function lin_srvPathToDisk($relPath) {
    // stored as 'lineage/anatella/<idserver>/<id>.anatella', relative to marvin/
